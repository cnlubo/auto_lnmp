...
INSTALL_MariaDB()
{
    echo "${CMSG}[mariadb${mariadb_10_0_version} Installing] **************************************************>>${CEND}";
    src_url=http://mirrors.opencas.cn/mariadb//mariadb-$mariadb_10_0_version/source/mariadb-$mariadb_10_0_version.tar.gz
    Download_src
    cd $script_dir/src
    [ -d mariadb-$mariadb_10_0_version ] && rm -rf mariadb-$mariadb_10_0_version
    tar -zxf mariadb-$mariadb_10_0_version.tar.gz;
    cd mariadb-$mariadb_10_0_version;
