#!/bin/bash
# @Author: cnak47
# @Date: 2020-04-06 09:50:16
# @LastEditors: cnak47
# @LastEditTime: 2020-04-06 09:50:50
# @Description:
# #
opt_tcp() {
    #######################################################
    ...
    if [[ ! -f /proc/user_beancounters ]]; then
        if [[ "$CENTOS_SEVEN" = '7' ]]; then
            if [ -d /etc/sysctl.d ]; then
                ...
                if [[ ! -z "$NF_CTHASHSIZE" && "$(
                    grep 'hashsize' /etc/rc.local >/dev/null 2>&1
                    echo $?
                )" != '0' ]]; then
                ...
                if [[ "$(
                    grep 'centminmod added' /etc/sysctl.d/101-sysctl.conf >/dev/null 2>&1
                    echo $?
                )" != '0' ]]; then
...
EOF
            ...
            # centos 6
            if [[ "$(
                grep 'centminmod added' /etc/sysctl.conf >/dev/null 2>&1
                echo $?
            )" != '0' ]]; then
                if [[ ! -z "$NF_CTHASHSIZE" && -f /sys/module/nf_conntrack/parameters/hashsize ]]; then
                    # raise hashsize for conntrack entries
