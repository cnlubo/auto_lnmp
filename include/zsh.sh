...
# @LastEditors: cnak47
# @LastEditTime: 2020-11-04 22:06:18
# @Description:
# #
            ...
            if [ "$CHECK_ZSH_INSTALLED" -ge 1 ]; then
                ...
                if [ -d $root_zsh ] || [ -d "$normal_zsh" ]; then
                    ...
                    if [ $default_user_exists -eq 1 ]; then
                        ...
                        INFO_MSG "nerd-fonts install ....."
                        if [ -d "$script_dir"/src/nerd-fonts ]; then
                            cd "$script_dir"/src/nerd-fonts || exit
                            sudo -u "${default_user:?}" -H ./install.sh Hack
                            sudo -u "${default_user:?}" -H ./install.sh Meslo
                        else
                        ...
                        if [ -f /home/"${default_user:?}"/.zshrc ] || [ -h /home/"${default_user:?}"/.zshrc ]; then
                            ...
                            # 设置新模版
                            sudo -u "${default_user:?}" -H sed -i "s@^#ZSH_THEME.*@&\nZSH_THEME=\"powerlevel10k/powerlevel10k\"@" /home/"${default_user:?}"/.zshrc
                            src_file=${script_dir:?}/template/zsh/Powerlevel10k
                            linenum=$(cat -n /home/"${default_user:?}"/.zshrc | grep 'ZSH_THEME="powerlevel10k/powerlevel10k"' | awk '{print $1}')
                            cat "${src_file}" | while read -r line; do
                                linenum=$((linenum + 1))
                                echo "$line" | sudo -u "${default_user:?}" -H sed -i "$linenum a $line" /home/"${default_user:?}"/.zshrc
                            done
                            # 增加配置
                            sudo -u "${default_user:?}" -H sed -i "s@^#ZSH_THEME.*@&\nPOWERLEVEL9K_MODE='nerdfont-complete'@" /home/"${default_user:?}"/.zshrc
                            sudo -u "${default_user:?}" -H sed -i "s@^#ZSH_THEME.*@&\nZSH_DISABLE_COMPFIX=true@" /home/"${default_user:?}"/.zshrc
                            ...
                            sudo -u "${default_user:?}" -H sed -i '$a\PATH=$PATH:/usr/sbin:$HOME/.local/bin:$HOME/bin\nexport PATH' /home/"${default_user:?}"/.zshrc
                    ...
                    if [ -f /root/.zshrc ] || [ -h /root/.zshrc ]; then
                        ...
                        # 设置新模版
                        sed -i "s@^#ZSH_THEME.*@&\nZSH_THEME=\"powerlevel10k/powerlevel10k\"@" /root/.zshrc
                        linenum=$(cat -n /root/.zshrc | grep 'ZSH_THEME="powerlevel10k/powerlevel10k"' | awk '{print $1}')
                        src_file=${script_dir:?}/template/zsh/Powerlevel10k
                        cat ${src_file} | while read -r line; do
                            linenum=$((linenum + 1))
                            echo "$line" | sed -i "$linenum a $line" /root/.zshrc
                        done
                        sed -i "s@^#ZSH_THEME.*@&\nPOWERLEVEL9K_MODE='nerdfont-complete'@" /root/.zshrc
